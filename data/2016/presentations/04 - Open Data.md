...
* Deux objectifs de cette dernière:   - Montrer   - Explorer
* Des questions?
...
* 2) Penser l’utilité potentielle de ces données sachant que:   - On ne peut jamais être certain de connaître toutes les utilisations possibles de ces données (croisements).   - L’on manque de temps, de compétences et de moyens pour exploiter ces données.
...
* Les données faisant tourner à elles seules votre commerce.
* Les données n’interessant personne d’autre (l’Open data, comme l’Open source, n’est pas une poubelle).
...
* Le format dans lequel ces données sont libérées (dans un monde idéal) s’adresse aux développeurs.

===

## Cible de l’Open data

* Les cibles primaires de l’Open data sont donc:  1) Les développeurs d’applicatifs 2) Les data journalists/scientists 3) Des citoyens connectés et avertis
...
* Beaucoup d’institutions n’ont pas les pré-requis pour assurer une libération de leurs données dans de bonnes conditions.
* Nombreuses limites légales (en France, notamment, étonnant non?)
...
* Divisée en deux:   - Droit moral (paternité, intégrité etc.)   - Droit patrimonial (revenus etc. spécifié)
...
* Démonstration par l’absurde: un mauvais exemple.
...
* Un Open data trop précipité et mal réalisé est néfaste:  - Peu voire aucun rendement - Effet de posture - Inutile - Problème d’image - On va jaser sur la toile (même l’effet Streisand a des limites)
...
* 1) Les données ne doivent pas être filtrées (i.e. originales et exhaustives) 2) Structurées 3) Libres 4) Urlisées 5) Liées à d’autres
...
* Déclarations d’intérêts des élus: http://regardscitoyens.org/interets-des-elus/
...
* Profils:   - Dev back (traitement de données)   - Chefs de projet spécialisés
* Compétences (hors HETIC):   - Connaissances en droit.
